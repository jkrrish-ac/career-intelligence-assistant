...
def test_history_is_empty_for_unseen_session():
    store = ConversationStore(max_turns_per_session=10)
    assert store.get_history("new-session") == []


def test_append_preserves_order_within_a_session():
    store = ConversationStore(max_turns_per_session=10)
    store.append("s1", "user", "What skills am I missing?")
    store.append("s1", "assistant", "You're missing Kubernetes experience.")
...
def test_sessions_are_isolated():
    store = ConversationStore(max_turns_per_session=10)
    store.append("s1", "user", "hello from s1")
    store.append("s2", "user", "hello from s2")

    assert store.get_history("s1") == [{"role": "user", "content": "hello from s1"}]
    assert store.get_history("s2") == [{"role": "user", "content": "hello from s2"}]


def test_history_is_capped_at_max_turns_and_drops_oldest_first():
    # max_turns_per_session=2 -> 2 user+assistant pairs -> 4 stored entries max
    store = ConversationStore(max_turns_per_session=2)
    for i in range(4):
        store.append("s1", "user", f"question {i}")
        store.append("s1", "assistant", f"answer {i}")
